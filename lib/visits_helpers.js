...
exports.withDistancesOfFriends = function(fb_token, loc, callback) {
  ...
  fb.withFriends(fb_token, function(err, friends) {
    
    if (err) {
      console.log("withFriends returned error");
      return;
    }
    ...
    for(var i=0;i<friends.length;i++) {
      var f = friends[i];
      friend_ids.push(Number(f['id']));
      friend_names[f['id'].toString()] = f['name'];
    }
    ...
    withVisitsNearLocation(loc, friend_ids, function(visits) {
      ...
      for(key in dist_map)
      {
        nearby_friends.push({
          ...
          , facebook_id: key
          , name : friend_names[key.toString()]
        });
      }
